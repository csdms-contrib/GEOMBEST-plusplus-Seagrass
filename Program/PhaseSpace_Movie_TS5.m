function PhaseSpace_Movie_TS5(RI1,RI2,RSLR1,RSLR2,TS,Title)
% Min RivInput, Max RivInput, Min RSLR, Max RSLR, Time Steps, Movie Title

%  25 March 2019 - RUNS WITH A TIME STEP OF 5 YEARS
...
initBSF = 56.75; % SG, PBC 50%


fprintf(2,'Warning: Phase space is enlarged to 10:80');
...
for RSLR = RSLR1:RSLR2
    ...
    ts = t' * 5; % Convert vector to column, and multiply by the length of time step
...
for RivInput = RI1:10:RI2                                                        % <-----------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Change back to 6
...
%         [X,finalwidth] = evalc('MarshWidth(filethread,filethread)');
        ...
        meterstep = ceil(1/(filethread/200))+10;
...
%         [X,finalwidth] = evalc('MarshWidth(filethread2,filethread)');
        ...
        ri = RivInput/10;        %# HARD-WIRED!!                                                 <---------------------------!!!!!!!!!!!!!!!!! Change back to 6
        for step = 1:TS+10
...
for step = 1:(TS+10)
    ...
    Riv = Riv*10;        %# HARD-WIRED!!                                     <-----------------------------------!!!!!!!!!!!!!!!!!!!!! Change back to 6
