...
while flag == 1
    ...
    if exist (filename) == 2 
        ...
        for row = 1:T                
                ...
                runfiles(M).export(row) = numbers(row + 2,10); % IR - export flux
                runfiles(M).erosioncoeff(row) = numbers(row + 2,11); % RL - erosion coefficient for marsh edge erosion
                runfiles(M).decaycoeff(row) = numbers(row + 2,12); % IR - coefficient for wave height decay over seagrass meadow
                runfiles(M).seagrass(row) = numbers(row + 2,13); % IR - binary determination for presence or absence of seagrass (1=seagrass, 0=no seagrass)
                runfiles(M).PBC(row) = numbers(row + 2,14); % IR - percent bay cover of seagrass meadow
        end         
    else 
